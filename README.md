...
6. __Illumination correction__.
7. __Sky subtraction__.
8. Alignment of __science frames__.
...
To address cosmetic defects, the __LACosmic algorithm__ is used to handle cosmic ray removal. The BPMs are applied to each CCD at the beginning of the 
reduction process.
...
- Applied BPM to each frame and filled the values using a directional polynomial inpaint algorithm.

B. __Aligned frames__:
- Both sky-included and/or sky-subtracted versions.
...
## Project Structure
    ...
    thilos/
        BPM/
            HCAM_BPM_CCD1_bin1.fits -> BPMs for binning 1x1
            HCAM_BPM_CCD2_bin1.fits
            HCAM_BPM_CCD3_bin1.fits
            HCAM_BPM_CCD4_bin1.fits
            HCAM_BPM_CCD5_bin1.fits 
            HCAM_BPM_CCD1_bin2.fits -> BPMs for binning 2x2
            HCAM_BPM_CCD2_bin2.fits
            HCAM_BPM_CCD3_bin2.fits
            HCAM_BPM_CCD4_bin2.fits
            HCAM_BPM_CCD5_bin2.fits
        
        script/
            directional_polynomial_inpaint.py -> This is a script to fill the empty values generated by the BPM. Each pixel is assigned a new value determined from adjacent pixels.
            fringe_mask.py -> Using imaging frames, this script generates a fringe map that is applied to each frame for Sloan z-band frame.
            rename_joinup_fits.py -> The joinup always saves the FITS files with a new structure name that is incompatible with THILOS. This script solves this by changing them to a new structure name that varies depending on the frame type.
        ...
        rebinning.py       -> The skyflats are always in binning 1x1. This script allows you to modify the binning.
